...
void Client::Initialise(std::string proxyString)
{
	...
	//Begin version check
	versionCheckRequest = http_async_req_start(NULL, "http://77.169.84.189/TPT/Mod/CheckVersion.json", NULL, 0, 0);
	otherStuffCheckRequest = http_async_req_start(NULL, "http://" SERVER "/Startup.json", NULL, 0, 0);
...
void Client::Tick()
{
	...
	if(otherStuffCheckRequest && http_async_req_status(otherStuffCheckRequest))
	{
		int status;
		int dataLength;
		char * data = http_async_req_stop(otherStuffCheckRequest, &status, &dataLength);
		otherStuffCheckRequest = NULL;
	...
	if (versionCheckRequest && http_async_req_status(versionCheckRequest))
	{
		...
		else if (data)
		{
			...
			try
			{
				...
				if (stableMajor.Value()>MOD_VERSION || (stableMinor.Value()>MOD_MINOR && stableMajor.Value() == MOD_VERSION) || stableBuild.Value()>MOD_BUILD)
				{
