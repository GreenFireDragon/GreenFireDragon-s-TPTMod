...
void SearchController::Update()
{
	if(!nextQueryDone && nextQueryTime < gettime())
	{
		...
		if (targetPage > searchModel->GetPageCount())
		{
			targetPage = searchModel->GetPageCount();
			searchModel->UpdateSaveList(searchModel->GetPageCount(), nextQuery);
		}
		searchModel->UpdateSaveList(targetPage, nextQuery);
	}
	searchModel->Update();
...
void SearchController::PrevPage()
{
	...
	{
		targetPage = searchModel->GetPageNum() - 1;
...
void SearchController::GotoPage(string Number, bool now)
{
	int num = atoi(Number.c_str());
	targetPage = num;
	nextQuery = searchModel->GetLastQuery();
	if (!now)
	{
		nextQueryTime = gettime() + 1000;
		nextQueryDone = false;
	}
	else
	{
		nextQueryDone = true;
		targetPage = num;
		searchModel->UpdateSaveList(num, nextQuery);
	}
}
...
void SearchController::RandomSave()
{
	activePreview = new PreviewController(searchModel->GetRandomSave(), false, new OpenCallback(this));
	ui::Engine::Ref().ShowWindow(activePreview->GetView());
}
...
void SearchController::removeSelectedC()
{
	...
	ClearSelection();
	targetPage = searchModel->GetPageNum();
