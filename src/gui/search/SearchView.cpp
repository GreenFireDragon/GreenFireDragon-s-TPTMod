	...
	c(NULL)
{
	...
	pageBox = new ui::Textbox(ui::Point(260, WINDOWH-18), ui::Point(WINDOWW-520, 16), "");
	pageLabel = new ui::Label(ui::Point(230, WINDOWH - 18), ui::Point(WINDOWW - 520, 16), "Page ");
	fromPagesLabel = new ui::Label(ui::Point(365, WINDOWH - 18), ui::Point(WINDOWW - 520, 16), " from 1");
	...
	randButton = new ui::Button(ui::Point(1, WINDOWH - 18), ui::Point(75, 16), "Random Save");
	...
	class GotoPageAction : public ui::TextboxAction
	{
		SearchView * v;
	public:
		GotoPageAction(SearchView * _v) { v = _v; }
		void TextChangedCallback(ui::Textbox * sender)
		{
			v->c->GotoPage(sender->GetText());
		}
	};
	pageBox->SetInputType(pageBox->Number);
	pageBox->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	pageBox->Appearance.VerticalAlign = ui::Appearance::AlignMiddle;
	pageBox->SetActionCallback(new GotoPageAction(this));
	...
	pageLabel->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	pageLabel->Appearance.VerticalAlign = ui::Appearance::AlignMiddle;

	AddComponent(pageLabel);
	AddComponent(pageBox);

	fromPagesLabel->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	fromPagesLabel->Appearance.VerticalAlign = ui::Appearance::AlignMiddle;

	AddComponent(fromPagesLabel);
	...
	class RandomSaveAction : public ui::ButtonAction
	{
		SearchView * v;
	public:
		RandomSaveAction(SearchView * _v) { v = _v; }
		void ActionCallback(ui::Button * sender)
		{
			v->c->RandomSave();
		}
	};
	randButton->SetActionCallback(new RandomSaveAction(this));
	randButton->Appearance.HorizontalAlign = ui::Appearance::AlignCentre;
	randButton->Appearance.VerticalAlign = ui::Appearance::AlignMiddle;
...
SearchView::~SearchView()
{
	...
	RemoveComponent(randButton);
	RemoveComponent(pageLabel);
	RemoveComponent(pageBox);
	RemoveComponent(fromPagesLabel);
...
{
	std::stringstream curPage, fromPage;
	curPage << sender->GetPageNum();
	fromPage << " from " << (sender->GetPageCount());
	fromPagesLabel->SetText(fromPage.str());
	pageBox->SetText(curPage.str());
