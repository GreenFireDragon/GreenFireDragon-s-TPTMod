	...
	else if (showHud)
	{
		...
		if (sample.particle.type)
		{
			...
			if (showDebug)
			{
				...
				sampleInfo << ", DColour: " << sample.particle.dcolour;
				sampleInfo << ", Flags: " << sample.particle.flags;
		...
		else if (sample.isMouseInSim)
		{
			...
			{
				sampleInfo << "Empty, Temp: " << std::fixed << sample.AirTemperature - 273.15 << " C";
				sampleInfo << ", Pressure: " << std::fixed << sample.AirPressure;
			}
		}
		else
		...
		if (sample.particle.type)
		{
			if (showDebug)
			{
				...
				sampleInfo << "Life: " << std::fixed << sample.particle.life;
				sampleInfo << ", Tmp: " << sample.particle.tmp;
				sampleInfo << ", Tmp2: " << sample.particle.tmp2;
				sampleInfo << ", PAVG: " << std::fixed << sample.particle.pavg;
		...
		if (showDebug)
		{
			if (sample.particle.type)
			{
				...
				textWidth = Graphics::textwidth((char*)sampleInfo.str().c_str());
				g->fillrect(XRES - 20 - textWidth, 41, textWidth + 8, 13, 0, 0, 0, alpha*0.5f);
				g->drawtext(XRES - 16 - textWidth, 43, (const char*)sampleInfo.str().c_str(), 255, 255, 255, alpha*0.75f);
			}
	...
	if(showHud && introText < 51)
	{
		...
		if (showDebug)
		{
			...
			case 0:
				...
				break;
			case 1:
				airMode = "Pressure Off";
				break;
			case 2:
				airMode = "Velocity Off";
				break;
			case 3:
				airMode = "Off";
				break;
			...
			case 5:
				...
				break;
			case 6:
				gravityMode = "Unradial";
				break;
			}

			fpsInfo << "Air: " << airMode << " ,Gravity: " << gravityMode;

			textWidth = Graphics::textwidth((char*)fpsInfo.str().c_str());
			g->fillrect(12, 27, textWidth + 8, 14, 0, 0, 0, alpha*0.5f);
			g->drawtext(16, 30, (const char*)fpsInfo.str().c_str(), 32, 216, 255, alpha*0.75f);
		}
	}
