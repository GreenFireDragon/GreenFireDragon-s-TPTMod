...
void GameModel::BuildQuickOptionMenu(GameController * controller)
{
	...
	quickOptions.push_back(new HeatOption(this));
	quickOptions.push_back(new WaterEquaOption(this));
...
void GameModel::SetWaterEnable(bool water)
{
	sim->water_equal_test = water;
	UpdateQuickOptions();
	if (water)
		SetInfoTip("Water Equalisation: On");
	else
		SetInfoTip("Water Equalisation: Off");
}

bool GameModel::GetWaterEnable()
{
	return sim->water_equal_test;
}

void GameModel::SetHeatEnable(bool heat)
{
	sim->legacy_enable = heat;
	UpdateQuickOptions();
	if (!heat)
		SetInfoTip("Heat Simulation: On");
	else
		SetInfoTip("Heat Simulation: Off");
}

bool GameModel::GetHeatEnable()
{
	return sim->legacy_enable;
}
