...
void GameController::Tick()
{
	...
	if (gameModel->GetSimulation()->stickmanInPortal == true && gameModel->GetSimulation()->portalSaveID != 0)
	{
		OpenSavePreview(gameModel->GetSimulation()->portalSaveID, 0, true);
	}
...
{
	gameModel->GetSimulation()->gravityMode = (gameModel->GetSimulation()->gravityMode+1)%7;

	switch (gameModel->GetSimulation()->gravityMode)
	{
	case 0:
		gameModel->SetInfoTip("Gravity: Down");
		break;
	case 1:
	...
	case 2:
		...
		break;
	case 3:
		gameModel->SetInfoTip("Gravity: Left");
		break;
	case 4:
		gameModel->SetInfoTip("Gravity: Up");
		break;
	case 5:
		gameModel->SetInfoTip("Gravity: Right");
		break;
	case 6:
		gameModel->SetInfoTip("Gravity: Unradial");
		break;
	}
}
...
void GameController::ToggleWater()
{
	gameModel->SetWaterEnable(!gameModel->GetWaterEnable());
}

void GameController::ToggleHeat()
{
	gameModel->SetHeatEnable(!gameModel->GetHeatEnable());
}
...
int GameController::GetAHeat()
{
	return gameModel->GetSimulation()->aheat_enable;
}

int GameController::AirMode()
{
	return gameModel->GetSimulation()->air->airMode;
}

int GameController::GravityMode()
{
	return gameModel->GetSimulation()->gravityMode;
}
...
#else
			currentVersion = MTOS(MOD_VERSION) "." MTOS(MOD_MINOR) " Stable, Build " MTOS(MOD_BUILD);
#endif
