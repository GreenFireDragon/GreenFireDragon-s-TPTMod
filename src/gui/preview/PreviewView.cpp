...
void PreviewView::AttachController(PreviewController * controller)
{
	...
	//ostringstream convertu, convertd;
	//convertu << c->GetSave()->GetVotesUp();
	//convertd << c->GetSave()->GetVotesDown();

	//VotesUpLabel = new ui::Label(ui::Point(Position.X + (XRES / 2) - 100, Position.Y + (YRES / 2) - 2), ui::Point(70, 16), convertu.str());
	//AddComponent(VotesUpLabel);
	//VotesDownLabel = new ui::Label(ui::Point(Position.X + (XRES / 2) - 100, Position.Y + (YRES / 2) + 8), ui::Point(70, 16), convertd.str());
	//AddComponent(VotesDownLabel);
