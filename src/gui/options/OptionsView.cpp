...
OptionsView::OptionsView():
	...
	gravityMode->AddOption(std::pair<std::string, int>("Left", 3));
	gravityMode->AddOption(std::pair<std::string, int>("Up", 4));
	gravityMode->AddOption(std::pair<std::string, int>("Right", 5));
	gravityMode->AddOption(std::pair<std::string, int>("Unradial", 6));
	...
	class RandomRange : public ui::TextboxAction
	{
	public:
		OptionsView * v;
	public:
		RandomRange(OptionsView * v_){ v = v_; }
		void TextChangedCallback(ui::Textbox * sender){ v->c->SetMaxSaveID(sender->GetText()); }
	};
	randomSaveRange = new ui::Textbox(ui::Point(Size.X - 180 - 8, Size.Y - 18), ui::Point(180, 16), "");
	randomSaveRange->SetInputType(ui::Textbox::Number);
	randomSaveRange->SetActionCallback(new RandomRange(this));
	AddComponent(randomSaveRange);

	tempLabel = new ui::Label(ui::Point(8, randomSaveRange->Position.Y), ui::Point(Size.X - 180 - 16, 16), "\bgRandom save max id:");
	tempLabel->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;	tempLabel->Appearance.VerticalAlign = ui::Appearance::AlignMiddle;
	AddComponent(tempLabel);
	...
	ui::Button * tempButton = new ui::Button(ui::Point(0, Size.Y - 1), ui::Point(Size.X, 16), "OK");
...
void OptionsView::NotifySettingsChanged(OptionsModel * sender)
{
	...
	randomSaveRange->SetText(sender->GetMaxSaveID());
}
