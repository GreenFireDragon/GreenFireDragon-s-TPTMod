...
int Element_STKM::run_stickman(playerst* playerp, UPDATE_FUNC_ARGS) {
	...
	if (((int)(playerp->comm) & 0x01) == 0x01)
	{
		...
		if (!moved && playerp->rocketBoots)
		{
			...
			for (int leg = 0; leg<2; leg++)
			{
				...
				int np = sim->create_part(-1, footX, footY, playerp->bootsElem);
				if (np >= 0)
				{
	...
	if (((int)(playerp->comm) & 0x04) == 0x04)
	{
		if (playerp->rocketBoots)
		{
			...
			{
				int footX = playerp->legs[leg * 8 + 4], footY = playerp->legs[leg * 8 + 5];
				int np = sim->create_part(-1, footX, footY + 1, playerp->bootsElem);
				if (np >= 0)
				{
	...
	if (x + rx >= 0 && y + ry>0 && x + rx<XRES && y + ry<YRES && (rx || ry))
	{
		...
		{
			if (!playerp->rocketBoots || (r & 0xFF) != playerp->bootsElem)
...
void Element_STKM::STKM_interact(Simulation * sim, playerst* playerp, int i, int x, int y)
{
	...
	if (r)
	{
		...
		if (sim->elements[r & 0xFF].HeatConduct && ((r & 0xFF) != PT_HSWC || sim->parts[r >> 8].life == 10) && ((playerp->elem != PT_LIGH && sim->parts[r >> 8].temp >= 323) || sim->parts[r >> 8].temp <= 243) && (!playerp->rocketBoots || (r & 0xFF) != playerp->bootsElem))
		{
			sim->parts[i].life -= 2;
			playerp->accs[3] -= 1;
		}
		...
		if ((r & 0xFF) == PT_STKP && (sim->parts[i].type == PT_STKM || sim->parts[i].type == PT_STKM2))
		{
			sim->portalSaveID = sim->parts[r >> 8].tmp2;
			sim->stickmanInPortal = true;
		}
