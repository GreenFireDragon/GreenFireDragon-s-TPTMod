...
int Simulation::Load(int fullX, int fullY, GameSave * save)
{
	...
	for(int n = 0; n < NPART && n < save->particlesCount; n++)
	{
		...
		if (parts[i].type == PT_STKM)
		{
			...
			if (spwn_i != 0)
			{
				player.elem = parts[spwn_i].ctype;
				player.rocketBoots = parts[spwn_i].tmp;
				player.bootsElem = parts[spwn_i].tmp2;
				parts[i].life = parts[spwn_i].life;
			}
			else
			{
				player.elem = PT_DUST;
				player.rocketBoots = false;
				player.bootsElem = PT_PLSM;
				parts[i].life = 100;
			}
		}
		else if (parts[i].type == PT_STKM2)
		{
			...
			if (spwn2_i != 0)
			{
				player2.elem = parts[spwn2_i].ctype;
				player2.rocketBoots = parts[spwn2_i].tmp;
				player2.bootsElem = parts[spwn2_i].tmp2;
				parts[i].life = parts[spwn2_i].life;
			}
			else
			{
				player2.elem = PT_DUST;
				player2.rocketBoots = false;
				player2.bootsElem = PT_PLSM;
				parts[i].life = 100;
			}
		}
...
void Simulation::init_can_move()
{
	...
	for (destinationType = 0; destinationType < PT_NUM; destinationType++)
	{
		if (destinationType != PT_BRCK && destinationType != PT_WATR && destinationType != PT_DSTW && destinationType != PT_CBNW && destinationType != PT_FRZW && destinationType != PT_SLTW && destinationType != PT_DMND && destinationType != PT_INSL && destinationType != PT_VOID && destinationType != PT_PVOD)
			can_move[PT_SWAV][destinationType] = 2;
	}
	...
	switch (t)
		{
			...
			case PT_STKM:
				...
				if (player.spwn == 0)
				{
					Element_STKM::STKM_init_legs(this, &player, i);
					player.spwn = 1;
					if (parts[spwn_i].tmp == 0)
					{
						player.rocketBoots = false;
					}
					else if (parts[spwn_i].tmp == 1)
					{
			...
			case PT_STKM2:
				...
				if (player2.spwn == 0)
				{
					...
					player2.spwn = 1;
					if (parts[spwn2_i].tmp == 0)
					{
						player2.rocketBoots = false;
					}
					else if (parts[spwn2_i].tmp == 1)
					{
			...
			case PT_FIGH:
			{
				...
				if (fcount < 100 && fighters[fcount].spwn==0)
				{
					...
					fighters[fcount].bootsElem = PT_PLSM;
					fighcount++;
					return i;
				}
				parts[i].type=0;
...
void Simulation::update_particles_i(int start, int inc)
{
	...
	if (elementCount[PT_LOVE] > 0 || elementCount[PT_LOLZ] > 0 || elementCount[PT_PING] > 0 || elementCount[PT_PONG] > 0) //LOVE and LOLZ element handling
	{
		...
		for (ny=0; ny<YRES-4; ny++)
		{
			for (nx=0; nx<XRES-4; nx++)
			{
				...
				else if ((ny<9 || nx<9 || ny>YRES - 7 || nx>XRES - 10) && (parts[r >> 8].type == PT_LOVE || parts[r >> 8].type == PT_LOLZ || parts[r >> 8].type == PT_PING || parts[r >> 8].type == PT_PONG))
					kill_part(r>>8);
				...
				else if (parts[r >> 8].type == PT_PING)
				{
					Element_PING::ping[nx / 16][ny / 5] = 1;
				}
				else if (parts[r >> 8].type == PT_PONG)
				{
					Element_PONG::pong[nx / 16][ny / 5] = 1;
				}
			}
		}
		for (nx=9; nx<=XRES-18; nx++)
		{
			for (ny=9; ny<=YRES-7; ny++)
			{
				...
				if (Element_PING::ping[nx / 16][ny / 5] == 1)
				{
					for (nnx = 0; nnx<16; nnx++)
					for (nny = 0; nny<5; nny++)
					{
						if (ny + nny>0 && ny + nny<YRES&&nx + nnx >= 0 && nx + nnx<XRES)
						{
							rt = pmap[ny + nny][nx + nnx];
							if (!rt&&Element_PING::RuleTable[nny][nnx] == 1)
								create_part(-1, nx + nnx, ny + nny, PT_PING);
							else if (!rt)
								continue;
							else if (parts[rt >> 8].type == PT_PING&&Element_PING::RuleTable[nny][nnx] == 0)
								kill_part(rt >> 8);

						}
					}
				}
				Element_PING::ping[nx / 16][ny / 5] = 0;
				if (Element_PONG::pong[nx / 16][ny / 5] == 1)
				{
					for (nnx = 0; nnx<16; nnx++)
					for (nny = 0; nny<5; nny++)
					{
						...
						{
							rt = pmap[ny + nny][nx + nnx];
							if (!rt&&Element_PONG::RuleTable[nny][nnx] == 1)
								create_part(-1, nx + nnx, ny + nny, PT_PONG);
							else if (!rt)
								continue;
							else if (parts[rt >> 8].type == PT_PONG&&Element_PONG::RuleTable[nny][nnx] == 0)
								kill_part(rt >> 8);

						}
					}
				}
				Element_PONG::pong[nx / 16][ny / 5] = 0;
			}
		}
	}
...
killed:
			...
			else
			{
				...
				if (!do_move(i, x, y, fin_xf, fin_yf))
				{
					...
					{
						s = 1;// position search direction (left/right first)
						r = (rand()%2)*2-1;
						...
						else if (elements[t].Falldown>1 && fabsf(pGravX*parts[i].vx+pGravY*parts[i].vy)>fabsf(pGravY*parts[i].vx-pGravX*parts[i].vy))
						{
							...
							for (j=0;j<rt;j++)
							{
								...
								case 2:
									pGravD = 0.01f - hypotf((x - XCNTR), (y - YCNTR));
									pGravX = ptGrav * ((float)(x - XCNTR) / pGravD);
									pGravY = ptGrav * ((float)(y - YCNTR) / pGravD);
									break;
								case 3:
									pGravX = -ptGrav;
									pGravY = 0.0f;
