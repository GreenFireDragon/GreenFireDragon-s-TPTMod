		...
		if (sim->parts[i].type && sim->parts[i].type >= 0 && sim->parts[i].type < PT_NUM) {
...

			{
				...
				if (pixel_mode & PSPEC_STICKMAN)
				{
					...
					Particle cpart = sim->parts[i];
					if (cpart.ctype == PT_NEUT || cpart.ctype == PT_ELEC || cpart.ctype == PT_PHOT || cpart.ctype == PT_PROT || cpart.ctype == PT_GRVT)
					{
						fire_r[ny / CELL][nx / CELL] = PIXR(sim->elements[cpart.ctype].Colour);
						fire_g[ny / CELL][nx / CELL] = PIXG(sim->elements[cpart.ctype].Colour);
						fire_b[ny / CELL][nx / CELL] = PIXB(sim->elements[cpart.ctype].Colour);
					}
